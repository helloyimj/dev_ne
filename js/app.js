...
// /* 체크박스 전체 선택 */
 $(document).ready(function(){
  ...
  // 전체 선택 클릭 시
  $("#check_all_01").click(function(){
    $("input[name=chkbox01]").prop("checked", $(this).is(":checked"));
  });

  // 개별 체크박스 클릭 시
  $("input[name=chkbox01]").click(function(){
    let total = $("input[name=chkbox01]").length;
    let checked = $("input[name=chkbox01]:checked").length;

    // 전부 체크되면 전체 선택에도 체크, 아니면 해제
    $("#check_all_01").prop("checked", total === checked);
  });
